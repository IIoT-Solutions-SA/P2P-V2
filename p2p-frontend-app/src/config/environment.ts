...
const isDemoMode = window.location.hostname === 'peerlink.sa';

// Use environment variables if set, otherwise use smart detection based on hostname
export const API_BASE_URL = import.meta.env.VITE_API_BASE_URL ||
  (isProductionServer ? 'http://15.185.167.236:8000' :
   isDemoMode ? 'http://peerlink.sa:8000' :
   'http://localhost:8000');
...
console.log('🔍 Environment Detection:', {
  ...
  isDemoMode,
  API_BASE_URL,
  WEBSITE_BASE_URL: import.meta.env.VITE_WEBSITE_BASE_URL ||
    (isProductionServer ? 'http://15.185.167.236:5173' :
     isDemoMode ? 'http://peerlink.sa' :
     'http://localhost:5173')
});
